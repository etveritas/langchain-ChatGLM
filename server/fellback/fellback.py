...
def fellback_save(uuid: str = Body(..., description="该组对话唯一标识", examples=["2c24de88"]),
                  ...
                  prompt: str = Body(..., description="用户问题组装的Prompt", examples=["请根据以下内容回答问题：你好"]),
                  fellback: str = Body(..., description="用户反馈内容", examples=["回答的准确"]),
                  ):
    # 日志文件路径
    log_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../logs")
    try:
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        fell_logger.info(f"""uuid: {uuid} answer: {str([answer])} docs: {str(docs)}, prompt: {str([prompt])}, fellback: {str([fellback])}""")
